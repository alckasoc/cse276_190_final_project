...
class MinimalService(Node):
    ...
    def pup_callback(self, request, response):
        ...
        #Added: longer sleep time for turn duration to turn 90 degrees at 1 rad per sec
        turn_duration = 3.14159/(2*5)  #pi/(2*x) seconds at x rad per sec = pi/2 rad = 90 degrees
        ...
        if (request.command == 'move_forward'):
            velocity_cmd.linear.x = 12.0 # was 0.5    # .5 in the linear X direction moves us forward
            ...
            velocity_cmd.linear.x = -12.0  #was 0.5
            ...
            velocity_cmd.angular.z = 5.0 #was 1.0
            self.vel_publisher_.publish(velocity_cmd)
            self.get_logger().info('Publishing: "%s"' % request.command)
